@icon("res://addons/cm-gd/icons/NetTransformSync2D.svg")
extends Node2D
class_name NetTransformSync2D
...
var _sync_target_position: Vector2
var _sync_target_rotation: float
var _sync_target_scale: Vector2
var _net_t: float = 0
...
func _refresh_authority() -> void:
	...
	_has_authority = is_multiplayer_authority()
...
func _process(delta: float) -> void:
	...
	if _has_authority:
		...
		if sync_rotation:
			parent.rotation = lerp_angle(parent.rotation, _sync_target_rotation, delta * sync_speed)
		if sync_scale:
...
func broadcast_transform(snap: bool = false, to_peer: CMNetPeer = null) -> void:
	...
	if authority_mode == NetSync.NetSyncAuthorityMode.SHARED:
		tocall = _st_any
	if to_peer:
		tocall.rpc_id(to_peer.peer_id, parent.position, parent.rotation, parent.scale, snap)
	else:
		tocall.rpc(parent.position, parent.rotation, parent.scale, snap)

@rpc("any_peer", "unreliable_ordered")
func _st_any(pos: Vector2, rot: float, scl: Vector2, snap: bool) -> void:
	if authority_mode != NetSync.NetSyncAuthorityMode.SHARED: return
	_sync_transform(pos, rot, scl, snap)

@rpc("authority", "unreliable_ordered")
func _st_authority(pos: Vector2, rot: float, scl: Vector2, snap: bool) -> void:
...
func _sync_transform(pos: Vector2, rot: float, scl: Vector2, snap: bool) -> void:
